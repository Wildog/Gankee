    ...
    return [[RACSignal createSignal:^RACDisposable * _Nullable(id<RACSubscriber>  _Nonnull subscriber) {
        ...
        NSURLSessionDataTask *task = [_session dataTaskWithURL:url completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
            if (error) {
                ...
                } else if ([(NSNumber *)resp[@"error"] boolValue]) {
                    NSError *responseError = [NSError errorWithDomain:@"GKErrorDomain" code:-44 userInfo:@{NSLocalizedDescriptionKey: resp[@"msg"]}];
